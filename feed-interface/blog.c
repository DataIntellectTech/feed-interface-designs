...

{
	...
	while (keep_listening > 0) {
		logmessage(__FUNCTION__, "waiting for connection");
		...
		while ((keep_listening > 0) && c > 0) {
			mysleep(waittime);
			sprintf(client_message, " Hello world #### %i", sent);
			//Send the message back to client
			write_success = write(client_sock, client_message, strlen(client_message));
			if (write_success < 0) {
	...
	connected = 0;
	logmessage(__FUNCTION__, "listen thread closing as the listen value has changed.");
	return NULL;
